...
public class ProductionRecordQueryService {
    ...
    public <T> ProductionRecordDto queryByResultId(DatabaseConfig config, Class<T> entityClass, Long resultId) {
        ...
        try (HikariDataSource dataSource = buildDataSource(config)) {
            ...
            List<T> entities = jdbcTemplate.query(
                    "SELECT * FROM " + tableName.value() + " WHERE mo_process_step_production_result_id = ?",
